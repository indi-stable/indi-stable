...
| **CI (3rdparty)** | **Verified end to end, 2026-09-04.** `.github/workflows/3rdparty-release.yml` ran the full check → build-libs → build-drivers → smoke-test → promote loop on both distros and succeeded, publishing a real GitHub Release (`indi-stable-3rdparty-v2.2.4.1`, 54 assets) and pushing a real promotion commit. All 8 jobs passed on the **first** run, unlike core's five — because the two gate scripts and the 18-pin rewrite had all been exercised on real hardware first. One real defect did surface, and only from a genuine promotion: the RPM `%changelog` guard compared the rendered line including its date, so it added a duplicate `2.2.4.1-1` entry (fixed, `9676c74`). |
| **CI (pyindi-client)** | **Verified end to end, 2026-09-04.** `.github/workflows/pyindi-client-release.yml` ran check → build → smoke-test → promote on both distros and succeeded, publishing `indi-stable-pyindi-client-2.2.0` (2 assets) and pushing a real promotion commit. All 6 jobs passed on the first run. The `%changelog` fix from `9676c74` carried over correctly — exactly one `2.2.0-1` entry on both distros, where 3rdparty's first promotion produced two. |
| **CI (core)** | **Verified end to end, 2026-08-28.** `.github/workflows/core-release.yml` (core only, both distros — see `DESIGN.md`, "Release automation, v1") ran the full check → build-fedora → build-debian → smoke-test-fedora → smoke-test-debian → promote loop for real on GitHub-hosted runners and succeeded: a real commit on `development`, a real git tag (`indi-stable-core-v2.2.4.2`), a real GitHub Release with all six artifacts attached. Took five iterations to get there — three genuine bugs found and fixed along the way, none of them things `check-upstream-tag.sh`'s own dry-run test or reading the YAML by hand had caught (see the dated section below). |
